    ...
    try:
        ...
        if df.empty:
            ...
            if selected_option:
                ...
                with st.spinner("비즈니스 모델 해독 및 기관 수급망 분석 중..."):
                    try:
                        eng_desc, kor_desc = get_translated_profile(selected_ticker)
                        
                        # 💡 [핵심 패치] 월스트리트 스마트머니(기관/내부자) 수급 동향 추출
                        try:
                        ...
                        with st.container(border=True):
                            ...
                            sc1.metric("💼 기관 보유율 (Inst Own)", inst_own, help="전체 유통 주식 중 월가 기관투자자가 쥐고 있는 물량의 비율입니다. 높을수록 큰 자본이 들어와 있다는 뜻입니다.")
