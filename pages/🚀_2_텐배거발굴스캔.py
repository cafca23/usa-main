...
def get_ai_commentary(ticker, company, sector, eng_desc, strategy_name, api_key):
    ...
    # 1. 구글 서버에 현재 API 키로 사용 가능한 모델 목록을 요구함
    valid_models = []
    ...
    # 2. 사용 가능한 목록 중 가장 최신/빠른 모델을 자동으로 스나이핑
    target_model = valid_models[0] # 기본값: 목록의 첫 번째 모델
    ...
    # 3. 고정된 이름이 아닌, 시스템이 찾아낸 '진짜 이름'으로 모델 가동
